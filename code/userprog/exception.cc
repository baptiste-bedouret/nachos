...
static int copyStringToMachine(int address, int size, char *values)
{
	...
	while (index < size && values[index] != '\0' && resOfWriteMem)
	{
...
void ExceptionHandler(ExceptionType which)
{
	...
	case SyscallException:
	{
		...
		{
			DEBUG('s', "GetString\n ");
			char read_buffer[MAX_STRING_SIZE];
			copyStringToMachine(machine->ReadRegister(4), strlen(read_buffer), read_buffer);
			int ch = consoledriver->GetString(read_buffer, std::min(MAX_STRING_SIZE, (int)strlen(read_buffer)));
			machine->WriteRegister(2, ch);
			break;
			/*
			char read_buffer[MAX_STRING_SIZE];
			DEBUG('s',"GetString\n ");
			consoledriver->GetString(read_buffer, std::min(MAX_STRING_SIZE,(int)strlen(read_buffer)));
			int from = copyStringToMachine(machine->ReadRegister(4),strlen(read_buffer),read_buffer);
			//machine->WriteRegister(2,from); 
